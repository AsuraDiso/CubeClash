...
namespace Bootstrap.UI
{
    public sealed class UiViewFactory : IUiViewFactory
    {
        ...
        public MainMenuScreenViews PopulateMainMenuScreens(MainMenuView mainMenuView)
        {
            ...
            for (var tab = MainMenuTab.Events; tab <= MainMenuTab.Settings; tab++)
            {
                ...
                switch (tab)
                {
                    case MainMenuTab.Home:
                        homeView = _objectResolver.Instantiate(_catalog.HomeScreenPrefab, screensRoot);
                        screen = homeView.transform as RectTransform;
                        break;
                    case MainMenuTab.Deck:
                        deckView = _objectResolver.Instantiate(_catalog.DeckScreenPrefab, screensRoot);
                        screen = deckView.transform as RectTransform;
                        break;
                    default:
                        screen = _objectResolver.Instantiate(_catalog.GetMainMenuScreenPrefab(tab), screensRoot);
                        break;
                }
            ...
            return new MainMenuScreenViews(homeView, deckView);
        }

        public MatchmakingOverlayView CreateMatchmakingOverlayView() => _objectResolver.Instantiate(_catalog.MatchmakingOverlayViewPrefab);

...
namespace Game.Scripts.Core.Data.Cards
{
    ...
    {
        public static List<PlacedCard> AutoPack(IReadOnlyList<CardDefinition> cards, int columns, int rows,
            IReadOnlyList<int> catalogIndices = null)
        {
            ...
            for (var i = 0; i < cards.Count; i++)
            {
                ...
                var catalogIndex = catalogIndices != null && i < catalogIndices.Count ? catalogIndices[i] : -1;
        ...
        public static bool FitsWithinGrid(Vector2Int origin, CardFootprintSize footprint, int columns, int rows)
        {
            return origin.x >= 0 && origin.y >= 0 && origin.x + footprint.Columns <= columns &&
                   origin.y + footprint.Rows <= rows;
        }

        public static bool[,] BuildOccupiedGrid(IReadOnlyList<PlacedCard> placed, int columns, int rows)
        {
            var occupied = new bool[columns, rows];
        ...
        public static bool CanPlace(bool[,] occupied, Vector2Int origin, CardFootprintSize footprint)
        {
        ...
        public static bool TryFindSlot(bool[,] occupied, int columns, int rows, CardFootprintSize footprint, out Vector2Int origin)
        {
